...
def load_concat_base_features(split: str) -> np.ndarray:
    """Muat dan gabung (concat) L2-normalized embeddings dari
    siglip2b256 dan siglip1b256."""
    e2 = load_embeddings("siglip2b256", split)[0]
    e1 = load_embeddings("siglip1b256", split)[0]
    return concat_features([e2, e1])


def run_senior_spec_eval(X_v1: np.ndarray, folds_v1: pd.DataFrame,
    ...
    # 3. Adil v2 (Train di v2, Eval di v1 UTUH)
    ...
    return {
        "combo": "concat_siglip2b256_siglip1b256",
        "head": head_name,
...
def main():
    ...
    print("--- Loading Features Concat (SigLIP2-Base + SigLIP1-Base) ---")
    X_v1 = load_concat_base_features("train")
    X_v2 = align_to_subset(X_v1, folds_v1, folds_v2)

    res = run_senior_spec_eval(X_v1, folds_v1, X_v2, folds_v2, head_name="mlp", seed=CFG.seed)

    print("\n=== HASIL EKSPERIMEN SPESIFIKASI SENIOR ===")
